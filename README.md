...
- **`output`** — how many jobs to show in the terminal (`show: all | top10 | top50 | <number>`) and whether to write result files (`html`, `csv`). The terminal table is a capped view; the written files always hold the **full** list, sorted, with clickable links. Files land in `reports/` (git-ignored). Both the terminal view and the files **exclude postings that are already closed** (detected at fetch time — "no longer advertised", 404, etc.) and, honoring `search.posted_within_days`, postings older than the window — so you don't waste time on dead or stale listings. Run `/rank` to liveness-check every scraped posting (`/scrape` only verifies the ones it inspects closely).
