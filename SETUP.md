...
### Tuning `/scrape` and `/rank` (`job-search.config.yaml`)

Run settings for the search commands live in one commented file at the repo root, `job-search.config.yaml`. Open it and set:

- **`search.locations`** — where to look (e.g. `"Berlin, Germany"`, `"Remote"`).
- **`search.employment_types`** — leave empty for everything, or narrow to e.g. `[freelance, part-time]`. Recognized: `full-time`, `part-time`, `contract`, `freelance`, `temporary`, `internship`.
- **`search.posted_within_days`** / **`search.workplace_types`** — the recency window and remote/hybrid/onsite filter.
- **`output.show`** — how many jobs to show in the terminal (`all`, `top10`, `top50`, or a number). The written result files always contain the full list.
- **`output.formats`** — `html`, `csv`, or both. Files are written to `reports/` (git-ignored).
- **`portals.disabled`** — skip an installed portal for a run without editing its skill.
- **`sources.extra`** — add extra job sources (RSS feed, JSON API, `site:` search, or a careers page) beyond the installed portal CLIs. The file has a commented example for each type.

Every key is optional; a missing file or key just uses the built-in defaults. The result files are generated by `tools/export_jobs.py` — the first `/scrape` or `/rank` that writes them may prompt once to approve `python3 tools/export_jobs.py`. To pre-approve it, add `Bash(python tools/export_jobs.py:*)` and `Bash(python3 tools/export_jobs.py:*)` to `.claude/settings.json`, and add the same two entries to the `ALLOWED_PERMISSIONS` set in `tools/security_guards.py` so CI's permission guard stays green (that guard requires every `settings.json` permission to be mirrored there — the same paired-diff rule the shipped portal CLIs follow).

**Keep personal settings out of git.** `job-search.config.yaml` is tracked, so anything you put in it can be pushed. For personal locations, preferences, or sources, copy it to `job-search.config.local.yaml` and edit that — it is git-ignored and read in preference to the tracked file. Custom templates registered via `/add-template` (under `templates/cv/` and `templates/cover_letters/`) and a `private/` folder are git-ignored too. Note that your **profile** (`CLAUDE.md` and the `01`–`07` skill files) is written into *tracked* files, which `.gitignore` cannot protect — for a personal search, use a **private repository** (section 8) rather than a public fork.
