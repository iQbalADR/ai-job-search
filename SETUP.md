...
Every key is optional; a missing file or key just uses the built-in defaults. The result files are generated by `tools/export_jobs.py`, and marking a job applied uses `tools/mark_applied.py` — the first `/scrape` or `/rank` that runs either may prompt once to approve it. To pre-approve, add these to `.claude/settings.json`, and add the same entries to the `ALLOWED_PERMISSIONS` set in `tools/security_guards.py` so CI's permission guard stays green (it requires every `settings.json` permission to be mirrored there — the same paired-diff rule the shipped portal CLIs follow):

```
Bash(python tools/export_jobs.py:*)
